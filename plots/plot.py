...
def read_logs(folder_arr):
    ...
    :return elpsed_times_arr: array of shape (n_algs, n_iters, 5) holding total, fwd, select, eval, and comm time
    ...
    for i, folder in enumerate(folder_arr):
        ...
        elpsed_times = elpsed_times[:n_elems].to_numpy()
        size_arr[i] = n_elems

        if bounds_arr is None:
            bounds_arr = np.zeros((n_folders, n_elems, 2)) # lb, ub
            elpsed_times_arr = np.zeros((n_folders, n_elems, 8)) # total_time,fwd_time,select_time,eval_time
        elif n_elems > bounds_arr.shape[1]:
            old_n_elems = bounds_arr.shape[1]
            # resize
            new_bounds_arr = np.zeros((n_folders, n_elems, 2))
            new_elpsed_times_arr = np.zeros((n_folders, n_elems, 8)) 
            for j in range(i):
        ...
        elpsed_times_arr[i,:n_elems,:] = elpsed_times[:,:8]

    return bounds_arr, elpsed_times_arr, size_arr

def plot_bound_gap(date, prob_name, large_lam, fname=None, hide_legend=False):
    """
    ...
    for i in range(4):
        ...
        ax.plot(xs, bounds_arr[i,:n_iters,0], linestyle=lss_arr[i], label=name_arr[i], color=color_arr[i])
        ax.plot(xs, bounds_arr[i,:n_iters,1], linestyle=lss_arr[i], color=color_arr[i])
        if bounds_arr.shape[1] >= 500:
            print("%s iter=500, lb=%.3f ub=%.3f (gap=%.8f)" % (name_arr[i], bounds_arr[i,500-1,0], bounds_arr[i,500-1,1], 
                 (bounds_arr[i,500-1,1] - bounds_arr[i,500-1,0])))
        if bounds_arr.shape[1] >= 1000:
            print("%s iter=1000, lb=%.3f ub=%.3f (gap=%.8f)" % (name_arr[i], bounds_arr[i,1000-1,0], bounds_arr[i,1000-1,1],
                 (bounds_arr[i,1000-1,1] - bounds_arr[i,1000-1,0])))
        print("%s iter=2000, lb=%.3f ub=%.3f (%.8f)" % (name_arr[i], bounds_arr[i,n_iters-1,0], bounds_arr[i,n_iters-1,1],
              (bounds_arr[i,n_iters-1,1] - bounds_arr[i,n_iters-1,0])))

        print("%s final lb=%.8f ub=%.8f (gap=%.3f)" % (name_arr[i], bounds_arr[i,n_iters-1,0], bounds_arr[i,n_iters-1,1], bounds_arr[i,n_iters-1,1] - bounds_arr[i,n_iters-1,0]))
    ...
    for k,j_0 in zip(sddp_color_idx_arr,sddp_j_0_arr):
        ...
        ax.plot(xs, np.mean(inf_sddp_lb_arr[4:6], axis=0), linestyle=lss_arr[k+4], label=name_arr[k], color=color_arr[k])
        ax.plot(xs, np.mean(inf_sddp_ub_arr[4:6], axis=0), linestyle=lss_arr[k+4], color=color_arr[k])
        # 10% and 90% quantile
        ...
        if n_iters >= 500:
            print("%s iter=500, median lb=%.3f ub=%.3f (%.8f)" % (
                    name_arr[k], 
                    np.mean(inf_sddp_lb_arr[4:6,500-1]), 
                    np.mean(inf_sddp_ub_arr[4:6,500-1]),
                   (np.mean(inf_sddp_ub_arr[4:6,500-1]) - np.mean(inf_sddp_lb_arr[4:6,500-1]))
            ))
        if n_iters >= 1000:
            print("%s iter=1000, median lb=%.8f ub=%.8f (%.3f)" % (
                    name_arr[k], 
                    np.mean(inf_sddp_lb_arr[4:6,1000-1]), np.mean(inf_sddp_ub_arr[4:6,1000-1]),
                    (np.mean(inf_sddp_ub_arr[4:6,1000-1]) - np.mean(inf_sddp_lb_arr[4:6,1000-1]))
            ))
        print("%s final median iter=%d, median lb=%.8f ub=%.8f (gap=%.3f)" % (
                    name_arr[k], 
                    n_iters,
                    np.mean(inf_sddp_lb_arr[4:6,-1]), 
                    np.mean(inf_sddp_ub_arr[4:6,-1]),
                    (np.mean(inf_sddp_ub_arr[4:6,-1]) - np.mean(inf_sddp_lb_arr[4:6,-1]))
        ))
        print("%s final .10 lb=%.8f ub=%.8f (gap=%.3f)" % (
            name_arr[k], inf_sddp_lb_arr[1,-1], inf_sddp_ub_arr[1,-1], inf_sddp_ub_arr[1,-1] - inf_sddp_lb_arr[1,-1]))
        print("%s final .90 lb=%.8f ub=%.8f (gap=%.3f)" % (
            name_arr[k], inf_sddp_lb_arr[8,-1], inf_sddp_ub_arr[8,-1], inf_sddp_ub_arr[8,-1] - inf_sddp_lb_arr[8,-1]))
    ...
    ax.set(
        ...
        title="Upper and lower bound gaps\n" + r"for {} ($\lambda={}$)".format(prob_name, lmbda),
    )
    plt.tight_layout()
    if fname is None:
        plt.show()
...
def plot_bound_gap_hydrothermal(fname=None):
    ...
    experiment_log_folder = os.path.join(log_folder, "%s/exp_0" % date)
    name_arr = ["EDDP", "Inf-EDDP", "CE-Inf-EDDP", "Gap-Inf-EDDP", "CE-Inf-SDDP", "Cyc-SDDP"]
    idx_arr = [i_0+13] + list(range(i_0,i_0+1)) + [1 if large_lam else 38] + [i_0+2] + list(range(i_0+3,i_0+13)) + list(range(i_0+15,i_0+25))
    print(idx_arr)
    folder_arr = [os.path.join(experiment_log_folder, "run_%d" % i) for i in idx_arr]
    bounds_arr, elpsed_times_arr, size_arr = read_logs(folder_arr)
    ...
    ax.set(
        yscale='symlog', 
        xlabel="Oracle queries",
        title="Upper and lower bound gaps\n" + r"for {} ($\lambda={}$)".format("hydro-thermal", lmbda),
    )
    plt.tight_layout()
    if fname is None:
        plt.show()
...
def plot_riskadverse_newsvendor_bound_gap(large_lam, fname=None):
    date = "2025_04_23"
    plot_bound_gap(date, 'risk_adverse_newsvendor', large_lam, fname)

def plot_hydro_bound_gap(large_lam, fname=None, hide_legend=False):
    date = "2025_01_15"
    plot_bound_gap(date, 'hydro-thermal', large_lam, fname, hide_legend)
...
def plot_runtimes(date, prob_name, large_lam, fname=None):
    ...
    name_arr = ["Inf-EDDP", "CE-Inf-EDDP", "Gap-Inf-EDDP", "CE-Inf-SDDP", "Cyc-SDDP"]
    bounds_arr, elpsed_times_arr, size_arr = read_logs(folder_arr)
    size_arr = np.minimum(size_arr, max_iter)
    ...
    label_arr = ['',] * 12
    color_arr = ['black', 'blue', 'green', 'orange', 'purple']
    for i in range(5):
        j = 4-i
        ax.bar(x - (j-2)*width, total_times_arr[i,:], width, label=name_arr[i], hatch=hatch_arr[i], color=color_arr[i])
    ...
    ax.set(
        ...
        title=r"Runtime for %s ($\lambda=%s$) over %d subprobs" % (prob_name, lmbda, 1+np.min(size_arr)),
        yscale='log',
    )
...
def plot_riskadverse_newsvendor_runtimes(large_lam, fname=None):
    date = "2025_04_23"
    plot_runtimes(date, 'risk_adverse_newsvendor', large_lam, fname)
...
def print_optimality_gap(large_lam):
    ...
    if large_lam:
        i_long_0 = 2
        lmbda = 0.9906
        name_arr = ["Inf-EDDP", "CE-Inf-EDDP", "Gap-Inf-EDDP", "CE-Inf-SDDP", "EDDP", "Cyc-SDDP"]
        run_id_arr = list(range(24))
        iter_arr = [500, 2000, 42000]
    else:
        i_long_0 = 0
        i_0 = 24
        lmbda = 0.8
        name_arr = ["Inf-EDDP", "CE-Inf-EDDP", "Gap-Inf-EDDP", "CE-Inf-SDDP", "EDDP", "Cyc-SDDP", "GCE-Inf-EDDP"]
    ...
    print("-"*50)
    print("lambda=%s" % (0.9906 if large_lam else 0.8))
    print("-"*50)
    for i, name in enumerate(name_arr):
        ...
        for t in iter_arr:
            if reordered_size_arr[i] < t-1:
                ...
                print("(Last) iter %d: lb=%.2f ub=%.2f (gap=%.6e)" % (
                    reordered_size_arr[i], lb[t_star], ub[t_star], gap[t_star]))
                continue
            lb = reordered_bounds_arr[i,:t-1,0]
            ub = reordered_bounds_arr[i,:t-1,1]
            gap = ub-lb
            t_star = np.argmin(gap)
            print("Iter %d: lb=%.2f ub=%.2f (gap=%.6e)" % (t, lb[t_star], ub[t_star], gap[t_star]))
...
def print_oos_performance(prob_name):
    """ 
    Prints in/out-of-sample performance for both inventory and hydro with lambda=0.9906
    """
    ...
    run_id_arr = []
    name_arr = ["Inf-EDDP", "CE-Inf-EDDP", "Gap-Inf-EDDP", "CE-Inf-SDDP(0)", "Cyc-SDDP(0)"]
...
if __name__ == '__main__':
    plot_bound_gap_hydrothermal('hydro_gap_lambdasmall_guided.png')
    # plot_newsvendor_bound_gap(True, 'newsvendor_gap_lambdalarge.png')
    plot_hydro_bound_gap(True, 'hydro_gap_lambdalarge.png', hide_legend=True)
    # plot_newsvendor_runtimes(1, 'newsvendor_runtime_lambdalarge.png')
